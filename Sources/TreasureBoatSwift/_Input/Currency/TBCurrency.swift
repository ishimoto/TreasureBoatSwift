...
public extension View {
    ...
    func currency(_ currency: TBCurrency) -> some View {
        self
        #if os(iOS) || os(tvOS)
            .keyboardType(currency.decimalCount == 0 ? .numberPad : .decimalPad)
        #endif
            .modifier(TBCurrencyFieldModifier(currency: currency))
    }
...
public struct TBCurrencyFieldModifier: ViewModifier {
    
    @State public var text = ""
    public var currency: TBCurrency
    
    public init(currency: TBCurrency) {
...
public class TBCurrency: ObservableObject {
    ...
    public init(_ type: TBCurrencyType, title: String? = nil, decimalType: DecimalType = .dot, decimalCount: Int = 2) {
